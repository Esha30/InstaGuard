        ...
        {[...new Array(2)].map((_, index) => (
          <React.Fragment key={index}>
            {Customers.map(({ feedback, email, username }, i) => (
              <div key={i} className="card bg-white p-4 rounded-xl shadow hover:shadow-md transition">
                <div className="text-gray-800 text-sm md:text-base italic">“{feedback}”</div>
                <div className="flex items-center gap-3 mt-4">
                  <FaUserCircle className="text-4xl text-[#da0c50]" />
                  <div className="flex flex-col text-sm text-gray-600">
                    <span className="font-medium text-gray-800">{username || "Anonymous"}</span>
      ...
      try {
        ...
        if (data?.feedback) {
          const anonymized = data.feedback.map((fb: any) => ({
            ...
            email: fb.email || "anonymous@instaguard.ai", // Optional email
            username: fb.username || "User", // Optional username
          }));
    ...
    <section className="bg-gradient-to-b from-[#ffe4ec] to-white py-24">
      <div className="container">
        <div className="section-heading text-center">
          <div className="tag text-pink-600 font-semibold uppercase">Testimonials</div>
        ...
        <div className="flex justify-center gap-6 mt-12 [mask-image:linear-gradient(to_bottom,transparent,black_25%,black_75%,transparent)] max-h-[738px] overflow-hidden">
